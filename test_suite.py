...
async def run(key, method, format, **kwargs):
    ...
    async with websockets.connect(
        ...
        def my_reciever(message):
            words_to_check = ["uh", "um", "mhmm", "mm-mm", "uh-uh", "uh-huh", "nuh-uh", "like"]
            pattern = r'\b(?:' + '|'.join(re.escape(word) for word in words_to_check) + r')\b'
            had_undesirable_word =  bool(re.search(pattern, message, re.IGNORECASE))
            if had_undesirable_word:
                print("bad boy")
